...
namespace ServoTool
{
    ...
    {
        List<List<float>> DataList2 = new List<List<float>>();
        public FormPlotXY()
        {
            InitializeComponent();
        }
        ...
        private void Btn2_Load_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (ReadPath.ShowDialog() == System.Windows.Forms.DialogResult.OK && ReadPath.FileName != "")
                {
                    ...
                    sr.ReadLine();
                    //CB2_AxisX.Items.RemoveAt(StrArry.Length - 1);
                    //CB2_AxisY.Items.RemoveAt(StrArry.Length - 1);
                    while (!sr.EndOfStream)
                    {
                        ...
                        {
                            DataList2.Add(new List<float>());
                            DataList2[i].Add(float.Parse(StrArry[i]));
                        }
                    }
                    sr.Close();
